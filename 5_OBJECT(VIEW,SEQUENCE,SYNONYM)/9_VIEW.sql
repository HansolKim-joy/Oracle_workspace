...
-- 함수의 계산 결과도 다 달라지기 떄문에 안된다고 뜨는 에러 
...
-- DEPT_CODE에 WITH CHECK OPTION을 넣으면서 값을 수정 불가능하게 했는데 이에 위배되기 떄문에 할 수 없다는 에러 발생
...
SET EMP_NAME = '선동이'
WHERE EMP_ID = 200;
-- EMP_NAME에는 WITH CHECK OPTION을 넣어주지 않았기 떄문에 문제없이 UPDATE 가능
